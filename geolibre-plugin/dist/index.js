...
var CANDIDATES_SOURCE = "streamsnap-candidates";
var SELECTED_SOURCE = "streamsnap-selected";
var LINK_SOURCE = "streamsnap-link";
var SITE_SOURCE = "streamsnap-site";
var HIT_LAYER = "streamsnap-candidates-hit";
var SELECTED_LAYER = "streamsnap-selected-outline";
var LINK_LAYER = "streamsnap-link-line";
var SITE_LAYERS = ["streamsnap-site-halo", "streamsnap-site-dot"];
var SOURCES = [
	CANDIDATES_SOURCE,
	SELECTED_SOURCE,
	LINK_SOURCE,
...
var INTERNAL = { "geolibre:internal": true };
var EMPTY = {
	type: "FeatureCollection",
	features: []
};
var METRES_PER_DEGREE$1 = 111e3;
var DEFAULT_SELECTED_STYLE = {
	outlineColor: "#facc15",
	outlineWidth: 2
};
/** Transient review geometry; candidate segments remain visually unchanged. */
var MatchOverlay = class {
	map;
	onSelect;
	view = null;
	...
	handleClick = (event) => {
		const segmentIndex = this.map.queryRenderedFeatures(event.point, { layers: [HIT_LAYER] })[0]?.properties?.segmentIndex;
	...
	destroy() {
		...
		for (const source of SOURCES) if (this.map.getSource(source)) this.map.removeSource(source);
	}
	static radiusBounds([lng, lat], radius) {
		const dLat = radius / METRES_PER_DEGREE$1;
		const dLng = Math.min(180, radius / (METRES_PER_DEGREE$1 * Math.max(Math.abs(Math.cos(lat * Math.PI / 180)), 1e-6)));
		return [
			lng - dLng,
			lat - dLat,
			lng + dLng,
	...
	ensureLayers() {
		for (const id of SOURCES) if (!this.map.getSource(id)) this.map.addSource(id, {
			type: "geojson",
			data: EMPTY
		});
		this.addLayer({
	...
	addLayer(layer) {
		if (!this.map.getLayer(layer.id)) this.map.addLayer({
			...layer,
			metadata: INTERNAL
		});
	}
...
//#endregion
//#region node_modules/.pnpm/@turf+helpers@7.4.0/node_modules/@turf/helpers/dist/esm/index.js
...
//#endregion
//#region node_modules/.pnpm/@turf+invariant@7.4.0/node_modules/@turf/invariant/dist/esm/index.js
...
//#endregion
//#region node_modules/.pnpm/@turf+distance@7.4.0/node_modules/@turf/distance/dist/esm/index.js
...
//#endregion
//#region node_modules/.pnpm/@turf+meta@7.4.0/node_modules/@turf/meta/dist/esm/index.js
...
//#endregion
//#region node_modules/.pnpm/@turf+nearest-point-on-line@7.4.0/node_modules/@turf/nearest-point-on-line/dist/esm/index.js
...
var RBush = class {
	...
	_condense(path) {
		for (let i = path.length - 1, siblings; i >= 0; i--) if (path[i].children.length === 0) {
			if (i > 0) {
...
var METRES_PER_DEGREE = 111e3;
function prepareSites(features) {
	const sites = [];
...
function prepareSegments(features) {
	const segments = [];
	features.forEach((feature, featureIndex) => {
...
function describeAttributes(features) {
	return [...new Set(features.flatMap((feature) => Object.keys(feature.properties)))].map((name) => ({
		name,
		unique: hasUniqueValues(features, name)
	}));
}
var SegmentIndex = class {
	segments;
	tree = new RBush();
	...
	candidates(lngLat, radius) {
		const [lng, lat] = lngLat;
		const dLat = radius / METRES_PER_DEGREE;
		const latitudeScale = Math.abs(Math.cos(lat * Math.PI / 180));
		const dLng = Math.min(180, radius / (METRES_PER_DEGREE * Math.max(latitudeScale, 1e-6)));
		const found = [];
...
function automaticMatch(site, segments, candidate, streamPrimaryKey) {
	const automaticStreamKey = candidate ? primaryKeyValue(segments[candidate.segmentIndex].properties[streamPrimaryKey]) : null;
	return buildRow(site, segments, candidate, automaticStreamKey, automaticStreamKey);
}
/** Select a candidate while comparing it with the retained automatic result. */
function manualMatch(site, segments, candidate, automaticStreamKey, streamPrimaryKey) {
	return buildRow(site, segments, candidate, automaticStreamKey, primaryKeyValue(segments[candidate.segmentIndex].properties[streamPrimaryKey]));
}
/** Build the single finalized point layer, preserving source-site row order. */
function buildSnapped(sites, rows, sitePrimaryKey) {
	return {
		type: "FeatureCollection",
		features: sites.map((site) => {
			const row = rows[site.index];
			const coordinates = row.snap_lng === null ? site.lngLat : [row.snap_lng, row.snap_lat];
			return {
				type: "Feature",
...
function buildRow(site, segments, candidate, automaticStreamKey, snappedStreamKey) {
	if (!candidate) return {
...
function primaryKeyValue(value) {
	return typeof value === "string" || typeof value === "number" ? value : null;
}
function measure(segment, lngLat) {
...
function hasUniqueValues(features, name) {
	if (!features.length) return false;
	const values = /* @__PURE__ */ new Set();
	for (const feature of features) {
		const value = feature.properties[name];
		if (typeof value !== "string" && (typeof value !== "number" || !Number.isInteger(value)) || value === "") return false;
...
function listWorkspaceVectorLayers(app) {
	const layers = [];
	for (const layer of app.listLayers()) {
		const type = app.getLayerFeatures(layer.id).find((feature) => feature.geometry)?.geometry?.type;
		const geometry = type === "Point" ? "point" : type === "LineString" || type === "MultiLineString" ? "line" : null;
		if (geometry) layers.push({
			layerId: layer.id,
			name: layer.name,
			geometry
		});
...
function readWorkspaceLayer(app, layer) {
	return app.getLayerFeatures(layer.layerId);
}
function onWorkspaceChange(map, listener) {
...
var DEFAULT_MAX_SEGMENT_DISTANCE_M = 1e3;
var FIT_PADDING = 48;
var FIT_MAX_ZOOM = 18;
var StreamSnapController = class {
	app;
	map;
	...
	sites = [];
	segments = [];
	segmentByKey = /* @__PURE__ */ new Map();
	index = null;
	rows = [];
	candidates = [];
	current = 0;
	streamRevision = null;
	maxSegmentDistance = DEFAULT_MAX_SEGMENT_DISTANCE_M;
	...
	constructor(app, map) {
		...
		this.stopWatching = onWorkspaceChange(map, () => this.handleWorkspaceChange());
		this.refreshLayers();
	}
	subscribe(listener) {
	...
	snapshot() {
		...
		return {
			...
			matchedCount: this.rows.filter((row) => row !== null && row.snapped_stream_key !== null).length,
	...
	get canStartReviewing() {
		return Boolean(this.stage === "settings" && this.config.sites.id && this.config.streams.id && this.config.sites.primaryKey && this.config.streams.primaryKey);
	}
	get canFinalize() {
		return Boolean(this.stage === "review" && this.rows.length && this.rows.every((row) => row !== null));
	}
	refreshLayers() {
		if (this.stage === "review") return;
		const layers = listWorkspaceVectorLayers(this.app);
		const signature = layers.map((layer) => `${layer.layerId}:${layer.name}:${layer.geometry}`).join("|");
		if (signature === this.layerSignature) return;
		this.layerSignature = signature;
		this.layers = layers;
		const ids = new Set(layers.map((layer) => layer.layerId));
		for (const kind of ["sites", "streams"]) {
	...
	chooseStreams(layerId) {
		this.chooseLayer("streams", layerId);
	}
	toggleAttribute(kind, name) {
		const config = this.config[kind];
	...
	choosePrimaryKey(kind, name) {
		if (this.stage !== "settings") return;
		const config = this.config[kind];
		if (!config.attributes.find((candidate) => candidate.name === name)?.unique) return;
		config.primaryKey = name;
		config.visible.add(name);
		this.emit();
	}
	setMaxSegmentDistance(distance) {
	...
	goto(index) {
		...
		} else this.matchPreparedCurrent(true);
	}
	previous() {
	...
	autoMatchCurrent() {
		const streamsData = this.config.streams.id ? this.read(this.config.streams.id) : null;
		this.setStreamData(streamsData);
		this.matchPreparedCurrent();
	}
	matchPreparedCurrent(fit = false) {
		...
		this.updateCandidates();
		this.rows[this.current] = automaticMatch(site, this.segments, this.candidates[0] ?? null, streamPrimaryKey);
		this.message = "";
	...
	selectSegment(segmentIndex) {
		...
		const candidate = this.candidates.find((item) => item.segmentIndex === segmentIndex);
		const streamPrimaryKey = this.config.streams.primaryKey;
		if (!site || !row || !candidate || !streamPrimaryKey) return;
		this.rows[this.current] = manualMatch(site, this.segments, candidate, row.automatic_stream_key, streamPrimaryKey);
		this.showCurrent();
	}
	...
	finalize() {
		...
		const existingOutputs = this.app.listLayers().filter((layer) => layer.name === outputName);
		for (const layer of existingOutputs) this.app.unregisterExternalNativeLayer(layer.id);
		this.app.addGeoJsonLayer(outputName, buildSnapped(this.sites, rows, this.config.sites.primaryKey));
	...
	chooseLayer(kind, layerId) {
		...
		config.attributes = describeAttributes(kind === "sites" ? prepareSites(data) : prepareSegments(data));
		this.emit();
	}
	resetLayerConfiguration(kind) {
		const config = this.config[kind];
		config.id = null;
		config.attributes = [];
	...
	handleWorkspaceChange() {
		...
		const revision = featureRevision(streamsData, this.config.streams.primaryKey);
		if (revision === this.streamRevision) return;
		this.setStreamData(streamsData, revision);
		if (row?.manual_revised) {
			this.updateCandidates();
			this.showCurrent();
		} else this.matchPreparedCurrent();
	}
	showCurrent(fit = false) {
		const site = this.sites[this.current];
		if (!site || !this.index) {
			this.candidates = [];
			this.overlay.clear();
		...
		const row = this.rows[this.current];
		const selectedIndex = row?.snapped_stream_key === null || row?.snapped_stream_key === void 0 ? null : this.segmentByKey.get(row.snapped_stream_key) ?? null;
		const view = {
			site,
			segments: this.segments,
			candidates: this.candidates,
			selectedIndex,
			snapped: row?.snap_lng === null || row?.snap_lng === void 0 ? null : [row.snap_lng, row.snap_lat]
		};
		this.overlay.render(view);
		if (fit) this.map.fitBounds(MatchOverlay.radiusBounds(site.lngLat, this.maxSegmentDistance), {
			padding: FIT_PADDING,
	...
	setStreamData(features, revision = featureRevision(features, this.config.streams.primaryKey)) {
		...
		this.segmentByKey = /* @__PURE__ */ new Map();
		if (!primaryKey) return;
		for (const segment of this.segments) {
			const value = segment.properties[primaryKey];
			if (typeof value === "string" || typeof value === "number") this.segmentByKey.set(value, segment.index);
		}
	}
	updateCandidates() {
		const site = this.sites[this.current];
		this.candidates = site && this.index ? this.index.candidates(site.lngLat, this.maxSegmentDistance) : [];
	}
	outputName() {
		return `${this.layers.find((layer) => layer.layerId === this.config.sites.id)?.name ?? "Sites"}_snapped`;
	}
	hasSnappedOutput() {
		return this.app.listLayers().some((layer) => layer.name === this.outputName());
	}
	emit() {
		for (const listener of this.listeners) listener();
	}
};
function visibleAttributeNames(config) {
	return config.attributes.map((attribute) => attribute.name).filter((name) => config.visible.has(name));
}
function featureRevision(features, primaryKey) {
	features ??= [];
	const marker = (index) => {
		const feature = features[index];
		return feature ? String(feature.id ?? feature.properties?.[primaryKey ?? ""] ?? "") : "";
	};
	return `${features.length}:${marker(0)}:${marker(features.length - 1)}`;
}
//#endregion
//#region src/panel.ts
function renderPanel(container, controller) {
	...
	settings.append(sites.root, separator(), streams.root, separator(), field("Max stream segment distance (m)", distanceInput), startButton, settingsStatus);
	...
	const outlineWidth = numberInput(2, .5, .5);
	styleSection.append(field("Outline color", outlineColor), field("Outline width (px)", outlineWidth));
	...
	review.append(nav, separator(), siteSection, autoMatchButton, separator(), streamSection, separator(), styleSection, separator(), finalizeSection, reviewStatus);
	container.append(settings, review);
	sites.select.addEventListener("change", () => controller.chooseSites(sites.select.value));
	streams.select.addEventListener("change", () => controller.chooseStreams(streams.select.value));
	distanceInput.addEventListener("change", () => {
		if (!controller.setMaxSegmentDistance(Number(distanceInput.value))) distanceInput.value = String(controller.snapshot().maxSegmentDistance);
	});
	...
	position.addEventListener("keydown", (event) => {
		...
		if (Number.isInteger(index) && index >= 1 && index <= total) controller.goto(index - 1);
		else position.value = String(controller.snapshot().current + 1);
	});
	autoMatchButton.addEventListener("click", () => controller.autoMatchCurrent());
	outlineColor.addEventListener("input", () => controller.setSelectedOutlineColor(outlineColor.value));
	outlineWidth.addEventListener("change", () => {
		if (!controller.setSelectedOutlineWidth(Number(outlineWidth.value))) outlineWidth.value = String(controller.snapshot().selectedStyle.outlineWidth);
	});
	...
	const update = () => {
		...
		if (inSettings) {
			...
			fillLayerOptions(streams.select, state.streamLayers, state.streamsLayerId);
			renderAttributeList(sites.attributes, state.siteAttributes, state.siteVisibleAttributes, state.sitePrimaryKey, "sites", controller);
			renderAttributeList(streams.attributes, state.streamAttributes, state.streamVisibleAttributes, state.streamPrimaryKey, "streams", controller);
			if (document.activeElement !== distanceInput) distanceInput.value = String(state.maxSegmentDistance);
			startButton.disabled = !controller.canStartReviewing;
			settingsStatus.textContent = state.message || (!state.sitesLayerId || !state.streamsLayerId ? "Choose point and line layers." : !state.sitePrimaryKey || !state.streamPrimaryKey ? "Choose one unique primary key for each layer." : "");
			return;
		}
		renderReview(state, {
			position,
...
function renderReview(state, ui) {
	const site = state.site;
	if (!site) return;
	ui.position.max = String(state.rows.length);
	if (document.activeElement !== ui.position) ui.position.value = String(state.current + 1);
	...
	renderProperties(ui.siteInfo, site.properties, state.sitePrimaryKey, state.siteVisibleAttributes);
	const row = state.row;
	if (!row) ui.matchSummary.textContent = "Finding the nearest stream segment…";
	else if (row.snapped_stream_index === null) ui.matchSummary.textContent = `No segment within ${formatMetres(state.maxSegmentDistance)}.`;
	else ui.matchSummary.textContent = `${row.manual_revised ? "Manually revised" : "Automatic match"} · ${formatMetres(row.snap_distance_m)}`;
	renderCandidates(ui.candidateList, state, (segmentIndex) => ui.controller.selectSegment(segmentIndex));
	if (document.activeElement !== ui.outlineColor) ui.outlineColor.value = state.selectedStyle.outlineColor;
	if (document.activeElement !== ui.outlineWidth) ui.outlineWidth.value = String(state.selectedStyle.outlineWidth);
	ui.finalizeButton.disabled = !ui.controller.canFinalize;
	ui.finalizeButton.textContent = state.hasSnappedOutput ? "Regenerate snapped sites" : "Generate snapped sites";
	ui.matchedSummary.textContent = `${state.matchedCount} of ${state.rows.length} matched`;
	ui.reviewStatus.textContent = state.message;
}
function renderCandidates(list, state, onSelect) {
	list.replaceChildren();
	for (const candidate of state.candidates) {
		const segment = state.segments[candidate.segmentIndex];
		const selected = segment.properties[state.streamPrimaryKey] === state.row?.snapped_stream_key;
		...
		choice.append(textSpan(formatValue(segment.properties[state.streamPrimaryKey])), textSpan(formatMetres(candidate.distance), "streamsnap-muted streamsnap-tabular"));
		choice.addEventListener("click", () => onSelect(candidate.segmentIndex));
		item.append(choice);
		if (selected) {
			const properties = document.createElement("dl");
			properties.className = "streamsnap-properties streamsnap-candidate-properties";
			renderProperties(properties, segment.properties, state.streamPrimaryKey, state.streamVisibleAttributes, false);
			item.append(properties);
		}
		list.append(item);
	}
}
function renderAttributeList(list, attributes, visibleAttributes, primaryKey, kind, controller) {
	list.replaceChildren();
	...
	for (const attribute of attributes) {
		...
		eye.setAttribute("aria-label", `${visible.has(attribute.name) ? "Hide" : "Show"} ${attribute.name}`);
		eye.title = primaryKey === attribute.name ? "Primary keys are always shown" : `${visible.has(attribute.name) ? "Hide" : "Show"} attribute`;
		eye.append(eyeIcon(visible.has(attribute.name)));
		eye.addEventListener("click", () => controller.toggleAttribute(kind, attribute.name));
		...
		primary.title = attribute.unique ? "Use as primary key" : "Primary keys require unique, non-empty text or integer values";
		primary.addEventListener("change", () => controller.choosePrimaryKey(kind, attribute.name));
		row.append(eye, name, primary);
		list.append(row);
	}
}
function renderProperties(container, properties, primaryKey, visibleAttributes, includePrimary = true) {
	container.replaceChildren();
	const names = includePrimary ? [primaryKey, ...visibleAttributes.filter((name) => name !== primaryKey)] : visibleAttributes.filter((name) => name !== primaryKey);
...
function layerSettings(label, meta) {
...
function fillLayerOptions(select, layers, selected) {
	const signature = layers.map((layer) => `${layer.layerId}:${layer.name}`).join("|");
	if (select.dataset.signature !== signature) {
		select.dataset.signature = signature;
		select.replaceChildren(new Option(layers.length ? "Choose a layer…" : "No layers loaded", ""));
		for (const layer of layers) select.append(new Option(layer.name, layer.layerId));
	}
...
function eyeIcon(open) {
	...
	shape.setAttribute("d", open ? "M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Zm10 3a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" : "M3 3l18 18M10.6 5.2A11 11 0 0 1 12 5c6.5 0 10 7 10 7a14 14 0 0 1-2.1 3.1M6.6 6.6C3.6 8.6 2 12 2 12s3.5 7 10 7c1.8 0 3.3-.5 4.6-1.2M9.9 9.9a3 3 0 0 0 4.2 4.2");
	svg.append(shape);
	return svg;
}
...
function formatMetres(value) {
	...
	return value < 1e3 ? `${value.toFixed(1)} m` : `${(value / 1e3).toFixed(2)} km`;
}
//#endregion
//#region geolibre-plugin/plugin.json
var id = "streamsnap";
var name = "StreamSnap";
var version = "0.1.0";
//#endregion
...
*/
var PANEL_ID = "streamsnap-panel";
var controller = null;
var unregisterPanel = null;
var plugin = {
	id,
	name,
	version,
	activate(app) {
		controller = new StreamSnapController(app, app.getMap());
